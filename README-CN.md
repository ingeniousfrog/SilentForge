...
SilentForge 读取 README、元数据、文件树、Release 与轻量代码信号，输出可预览、打包 ZIP、任意静态托管的自包含 HTML。提供 **`reposite` / `silentforge` CLI** 一次性生成，或本地 **Workbench** 交互预览与 GitHub Pages 部署。

流水线**确定且以仓库事实为准**——无证据的章节自动省略。可选 OpenAI 规划可调整叙事顺序，但每条信息必须能回溯到已提取数据。

[English](./README.md) · **[在线演示](https://ingeniousfrog.github.io/SilentForge/)** — 由本仓库自动生成的展示站
...
**在线演示：** [SilentForge GitHub Pages 展示站](https://ingeniousfrog.github.io/SilentForge/) — 每次 push 到 `main` 后由**本仓库**自动重新生成（[workflow](./.github/workflows/silentforge-pages.yml)），可作为部署后效果的参考样板。
...
- **本地 Workbench** — 粘贴 URL、SSE 任务流、**Overview / Resources / Code Wiki / Preview** 四 Tab、ZIP 下载、**Deploy** 复制 GitHub Pages workflow
...
[`.github/workflows/silentforge-pages.yml`](./.github/workflows/silentforge-pages.yml) 已提交到本仓库。每次 push 到 `main` 会：
...
1. 为本仓库启用 Pages：[Settings → Pages](https://github.com/ingeniousfrog/SilentForge/settings/pages) → **Source: GitHub Actions**。
2. 在 **Actions** 中运行 **Deploy SilentForge presentation site**（或重新运行最近一次失败的工作流）。
3. **build** 与 **deploy** 均成功后，访问 **https://ingeniousfrog.github.io/SilentForge/**。

之后每次 push 到 `main` 会自动重新生成并发布。URL 可访问后，可在 README 顶部保留 Live demo badge。
...
这是 SilentForge **对本仓库自动生成的展示站**，用来看输出效果，不是单独的官网。若 URL 返回 404，请先在 **Settings → Pages → GitHub Actions** 启用 Pages，并重新运行 [Deploy SilentForge presentation site](https://github.com/ingeniousfrog/SilentForge/actions/workflows/silentforge-pages.yml)（见[本仓库演示站](#本仓库演示站silentforge-demo)）。
