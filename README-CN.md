...
SilentForge 以 Node.js 包形式交付。**`reposite`** 命令是 `package.json` 中注册的编译入口：

```json
"bin": { "reposite": "./dist/cli.js" }
```

源码位于 `src/cli.ts`。执行 `npm run build` 将 TypeScript 编译到 `dist/`，提供两个子命令：

| 命令 | 用途 |
|------|------|
| `reposite init <url>` | 生成静态站点到本地目录 |
| `reposite web` | 在 `127.0.0.1:4177` 启动 Workbench |
